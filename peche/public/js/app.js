...
(function () {
  ...
  let state = { trips: [], tracks: [], settings: {}, curTrack: null, openId: null, nemo: false, user: null };
  ...
  async function tryLogin() {
    ...
    try {
      ...
      state.user = me;
      await boot();                    // charge les données avant de basculer
      // Transition douce : fondu du login, apparition de l'appli
  ...
  function applyRoleUI() {
    const adminOnly = ["carte", "controle", "reglages"];
  ...
  function renderHead() {
    ...
    uInfo.innerHTML = `${esc(state.user.username)} (${state.user.role === "admin" ? "admin" : "pêcheur"}) · <button id="logoutBtn" style="background:none;border:none;color:var(--lagoon);cursor:pointer;font-size:11.5px;padding:0;text-decoration:underline">déconnexion</button>`;
  ...
  function openEditModal(t) {
    ...
    modal.innerHTML = `
      <div class="modal">
        <h2 class="fr" style="margin:0 0 4px">Modifier la sortie</h2>
        <p class="lead" style="margin:0 0 16px">Correction réservée à l'administratrice.</p>
        <label>Date</label><input type="date" id="e-date" value="${esc(t.date)}">
        <div class="two">
          <div><label>Heure départ</label><input type="time" id="e-dep" value="${esc(t.depart || "")}"></div>
          <div><label>Heure retour</label><input type="time" id="e-ret" value="${esc(t.retour || "")}"></div>
        </div>
    ...
    el2(modal, "e-save").addEventListener("click", async () => {
      ...
      const patch = {
        date: el2(modal, "e-date").value, depart: el2(modal, "e-dep").value,
  ...
  function renderSaisie(v) {
    ...
    v.innerHTML = `
      <div class="form">
        ...
        <p class="lead">À remplir au retour, après la pesée au débarquement. Une fois enregistrée, seule l'administratrice peut la corriger.</p>
  ...
  async function saveTrip() {
    ...
    } catch (e) { w.hidden = false; w.textContent = e.message; btn.disabled = false; btn.textContent = "Enregistrer la sortie"; }
  }
