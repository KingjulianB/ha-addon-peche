...
(function () {
  ...
  el("goRegister").addEventListener("click", () => { el("login").hidden = true; el("register").hidden = false; });
  el("goLogin").addEventListener("click", () => { el("register").hidden = true; el("login").hidden = false; });
  ...
  async function doJoin() {
    ...
    const pw = el("j-pw").value;
    const btn = el("joinBtn"); const errBox = el("joinErr"); const okBox = el("joinOk");
    errBox.hidden = true; okBox.hidden = true;
    if (!code || !email || !pw) { errBox.hidden = false; errBox.textContent = "Tous les champs sont requis."; return; }
    btn.disabled = true; btn.textContent = "Création…";
    try {
      const r = await API.join(code, email, pw);
      okBox.hidden = false;
      okBox.innerHTML = `Compte créé ✓ Vous avez rejoint <b>${esc(r.entreprise)}</b>.<br>Vous pouvez maintenant vous connecter.`;
      btn.textContent = "Rejoindre"; btn.disabled = false;
      setTimeout(() => { el("join").hidden = true; el("login").hidden = false;
        el("usr").value = email; el("loginErr").hidden = true; }, 2500);
    } catch (e) {
      errBox.hidden = false; errBox.textContent = e.message;
  ...
  async function doRegister() {
    const nom = el("r-nom").value.trim();
    const email = el("r-email").value.trim();
    const pw = el("r-pw").value;
    const btn = el("registerBtn"); const errBox = el("registerErr"); const okBox = el("registerOk");
    errBox.hidden = true; okBox.hidden = true;
    if (!nom || !email || !pw) { errBox.hidden = false; errBox.textContent = "Tous les champs sont requis."; return; }
    btn.disabled = true; btn.textContent = "Création…";
    try {
      const r = await API.register(nom, email, pw);
      okBox.hidden = false;
      if (r.email_envoye) {
        okBox.innerHTML = `Entreprise créée ✓<br>Un email de vérification a été envoyé à <b>${esc(email)}</b>. Ouvrez-le pour activer votre compte, puis connectez-vous.`;
      } else {
        // Mode sans SMTP : on donne le lien de vérification directement
        okBox.innerHTML = `Entreprise créée ✓<br>Vérification (envoi email non configuré) — ouvrez ce lien pour activer votre compte :<br><a href="${esc(r.lien_verification)}" style="color:var(--lagoon);word-break:break-all">${esc(r.lien_verification)}</a>`;
      }
      btn.textContent = "Créer mon entreprise"; btn.disabled = false;
    } catch (e) {
      errBox.hidden = false; errBox.textContent = e.message;
  ...
  // --- Gestion du lien de vérification (?token=...) ---
  (async function handleVerifyLink() {
    ...
    try {
      ...
      // affiche un message de succès sur l'écran de login
      el("login").hidden = false; el("register").hidden = true;
      const ok = el("loginErr");
      ok.hidden = false; ok.style.color = "var(--lagoon)";
      ok.textContent = `Email ${r.email} vérifié ✓ Vous pouvez maintenant vous connecter.`;
    } catch (e) {
      el("loginErr").hidden = false; el("loginErr").textContent = e.message;
    }
    // nettoie l'URL
  ...
  const isSuperAdmin = () => state.user && state.user.superadmin;
  function applyRoleUI() {
    const adminOnly = ["carte", "controle", "depenses", "pannes", "tresorerie", "equipe", "bilan", "reglages"];
    document.querySelectorAll(".tab").forEach((t) => {
      const tab = t.dataset.tab;
      if (tab === "entreprises") {
        // Onglet réservé au super-admin (l'éditeur de la plateforme)
        t.style.display = isSuperAdmin() ? "" : "none";
      } else if (!isAdmin() && adminOnly.includes(tab)) {
  ...
  function renderTab(name) {
    ...
    if (name === "pannes") return renderPannes(v);
    if (name === "tresorerie") return renderTresorerie(v);
    if (name === "bilan") return renderBilan(v);
    if (name === "entreprises") return renderEntreprises(v);
    if (name === "equipe") return renderEquipe(v);
  ...
  /* ================= PANNES (admin) ================= */
  let pannePhotos = []; // data URLs en attente
  async function renderPannes(v) {
    try { state.pannes = await API.pannes(); } catch (e) { /* garde */ }
    const list = state.pannes.slice();
    v.innerHTML = `
      <div class="form" style="max-width:640px">
        ...
        <div style="border:1px solid var(--line);border-radius:2px;padding:16px;margin-bottom:20px">
          ...
          <div class="two">
            <div><label>Date</label><input type="date" id="p-date" value="${todayISO()}"></div>
            <div><label>Coût réparation (FCFA)</label><input type="number" id="p-cout" inputmode="numeric" step="500" placeholder="ex : 45000"></div>
          </div>
          <label>Constat / description</label>
          <textarea id="p-constat" rows="3" placeholder="Décrivez le problème constaté…" style="font-family:inherit"></textarea>
          <label>Statut</label>
          <select id="p-statut"><option value="ouverte">Ouverte</option><option value="reparee">Réparée</option></select>
          <label>Photos du constat (plusieurs possibles)</label>
          <input type="file" id="p-photos" accept="image/*" capture="environment" multiple style="padding:0;border:none;background:none">
          <div id="p-photo-preview" style="display:flex;gap:8px;flex-wrap:wrap;margin-top:10px"></div>
          <button class="btn" id="p-add">Enregistrer la panne</button>
        </div>

        <p class="eyebrow" style="margin-bottom:6px">Historique des pannes</p>
        <div id="p-list"></div>
      </div>`;

    pannePhotos = [];
    el("p-photos").addEventListener("change", (e) => {
      const files = Array.from(e.target.files || []);
      const box = el("p-photo-preview");
    ...
    el("p-add").addEventListener("click", addPanne);
    renderPanneList();
  }

  function renderPanneList() {
    const list = el("p-list"); if (!list) return;
    ...
    list.innerHTML = items.map((p) => `
      <div class="entry" style="border:1px solid var(--line);border-radius:2px;padding:12px;margin-bottom:10px">
        ...
        <div style="margin:4px 0">
          <span class="badge ${p.statut === "reparee" ? "in" : "out"}">${p.statut === "reparee" ? "Réparée" : "Ouverte"}</span>
    ...
    list.querySelectorAll("[data-repare]").forEach((b) => b.addEventListener("click", async () => {
      try { await API.updatePanne(b.dataset.repare, { statut: "reparee" });
        state.pannes = await API.pannes(); renderPanneList(); toast("Panne marquée réparée");
      } catch (e) { toast(e.message); }
    }));
    list.querySelectorAll("[data-delpanne]").forEach((b) => b.addEventListener("click", async () => {
      if (!confirm("Supprimer cette panne ? (la dépense liée sera aussi retirée)")) return;
      try { await API.delPanne(b.dataset.delpanne);
        state.pannes = state.pannes.filter((x) => x.id !== b.dataset.delpanne);
        renderPanneList(); toast("Panne supprimée");
      } catch (e) { toast(e.message); }
    }));
  }

  async function addPanne() {
    const titre = el("p-titre").value.trim();
    if (!titre) { toast("Indiquez un titre."); return; }
    const panne = {
      titre, date: el("p-date").value || todayISO(),
      cout: parseFloat(el("p-cout").value) || 0,
      constat: el("p-constat").value.trim(),
      statut: el("p-statut").value, photos: pannePhotos,
    };
    const btn = el("p-add"); btn.disabled = true; btn.textContent = "Enregistrement…";
    try {
      await API.addPanne(panne);
      state.pannes = await API.pannes();
      pannePhotos = [];
      toast(panne.cout > 0 ? "Panne enregistrée (dépense créée)" : "Panne enregistrée");
      renderPannes(el("view"));
    } catch (e) { toast(e.message); btn.disabled = false; btn.textContent = "Enregistrer la panne"; }
  }

  /* ================= TRÉSORERIE (admin) ================= */
  async function renderTresorerie(v) {
    let t;
    try { t = await API.tresorerie(); } catch (e) { v.innerHTML = `<div class="empty">${esc(e.message)}</div>`; return; }
    const soldeColor = t.solde >= 0 ? "var(--lagoon)" : "var(--signal)";
    v.innerHTML = `
      <div class="map-wrap" style="max-width:560px">
        ...
        <table class="mini">
          <tr><td>Recettes (ventes de poisson)</td><td class="r num" style="color:var(--lagoon)">+ ${fcfa(t.recettes)} FCFA</td></tr>
          <tr><td>Dépenses (carburant, paye, pannes…)</td><td class="r num" style="color:var(--signal)">− ${fcfa(t.depenses)} FCFA</td></tr>
          <tr style="font-weight:600"><td>Solde</td><td class="r num" style="color:${soldeColor}">${fcfa(t.solde)} FCFA</td></tr>
        </table>
        <p class="hint" style="margin-top:14px">Le solde reflète l'ensemble des ventes déclarées moins toutes les dépenses (y compris les réparations de pannes). Il donne une image de ce qui devrait être en caisse.</p>
      </div>`;
  }

  /* ================= ENTREPRISES (super-admin) ================= */
  async function renderEntreprises(v) {
    let list;
    ...
    v.querySelectorAll("[data-activer]").forEach((b) => b.addEventListener("click", () => setStatut(b.dataset.activer, "actif")));
    v.querySelectorAll("[data-verifier]").forEach((b) => b.addEventListener("click", async () => {
  ...
  function entRow(e) {
    ...
    return `<div class="entry" style="border:1px solid var(--line);border-radius:2px;padding:14px;margin-bottom:10px">
      ...
      <div class="muted" style="font-size:12.5px;margin-top:4px">
        ${e.admin_email ? "Admin : " + esc(e.admin_email) : "Pas d'admin"}
        ${verifie ? ' · <span style="color:var(--lagoon)">email vérifié ✓</span>' : ' · <span style="color:var(--signal)">email non vérifié</span>'}
  ...
  async function setStatut(id, statut) {
    if (statut === "suspendu" && !confirm("Suspendre cette entreprise ? Ses comptes ne pourront plus se connecter.")) return;
    try { await API.setEntrepriseStatut(id, statut); toast(statut === "suspendu" ? "Entreprise suspendue" : "Entreprise réactivée"); renderEntreprises(el("view")); }
    catch (e) { toast(e.message); }
  }

  /* ================= ÉQUIPE & PIROGUES (admin) ================= */
  async function renderEquipe(v) {
    let pirogues = [], invitations = [];
    try { [pirogues, invitations] = await Promise.all([API.pirogues(), API.invitations()]); }
    catch (e) { v.innerHTML = `<div class="empty">${esc(e.message)}</div>`; return; }
    const dispo = invitations.filter((i) => !i.utilise);
    v.innerHTML = `
      <div class="form" style="max-width:640px">
        ...
        <div style="border:1px solid var(--line);border-radius:2px;padding:16px;margin-bottom:22px">
          ...
          <div style="display:flex;gap:8px;margin-top:12px">
            <input type="text" id="pir-nom" placeholder="ex : GA-PG-2214" style="flex:1">
            <button class="btn small" id="pir-add" style="margin:0">Ajouter</button>
          </div>
        </div>
        ...
        <div style="border:1px solid var(--line);border-radius:2px;padding:16px">
          ...
          <div style="margin-top:16px">
            <p class="eyebrow" style="margin-bottom:6px">Codes disponibles (${dispo.length})</p>
            <div id="inv-list">${invitations.map(invRow).join("") || '<p class="muted" style="font-size:13px">Aucun code généré.</p>'}</div>
          </div>
        </div>
      </div>`;

    el("pir-add").addEventListener("click", async () => {
      const nom = el("pir-nom").value.trim();
      if (!nom) { toast("Nom de la pirogue requis."); return; }
      try { await API.addPirogue(nom); toast("Pirogue ajoutée"); renderEquipe(v); }
      catch (e) { toast(e.message); }
    });
    el("inv-add").addEventListener("click", async () => {
      try { const r = await API.addInvitation(); toast("Code généré : " + r.code); renderEquipe(v); }
      catch (e) { toast(e.message); }
    });
    v.querySelectorAll("[data-delpir]").forEach((b) => b.addEventListener("click", async () => {
      if (!confirm("Supprimer cette pirogue ?")) return;
      try { await API.delPirogue(b.dataset.delpir); renderEquipe(v); } catch (e) { toast(e.message); }
    }));
    v.querySelectorAll("[data-delinv]").forEach((b) => b.addEventListener("click", async () => {
      try { await API.delInvitation(b.dataset.delinv); renderEquipe(v); } catch (e) { toast(e.message); }
    }));
    v.querySelectorAll("[data-copy]").forEach((b) => b.addEventListener("click", () => {
      const code = b.dataset.copy;
      if (navigator.clipboard) navigator.clipboard.writeText(code).then(() => toast("Code copié : " + code));
      else toast("Code : " + code);
    }));
  }

  function pirRow(p) {
    return `<div style="display:flex;justify-content:space-between;align-items:center;padding:8px 0;border-bottom:1px solid var(--paper-shade)">
      <span style="font-weight:500">${esc(p.nom)}</span>
      <button class="del" data-delpir="${p.id}" style="color:var(--signal)">Supprimer</button>
    </div>`;
  }
  function invRow(i) {
    return `<div style="display:flex;justify-content:space-between;align-items:center;padding:10px 0;border-bottom:1px solid var(--paper-shade)">
      <span class="fr num" style="font-size:20px;letter-spacing:3px;font-weight:700;${i.utilise ? "opacity:.4;text-decoration:line-through" : ""}">${esc(i.code)}</span>
      <span style="display:flex;gap:8px;align-items:center">
        ${i.utilise
          ? '<span class="badge in">Utilisé</span>'
          : `<button class="btn ghost small" data-copy="${esc(i.code)}">Copier</button>
             <button class="del" data-delinv="${esc(i.code)}" style="color:var(--signal)">Suppr.</button>`}
      </span>
    </div>`;
  }
    ...
    v.innerHTML = `
      <div class="form">
        ...
        <div class="two">
          ...
          <div><label>Prix du carburant du jour (FCFA/L)</label><input id="r-prix" type="number" step="10" value="${esc(s.fuel_prix || "700")}"></div>
        </div>
        <p class="hint">Consommation : valeur par défaut 7 L/h (hors-bord de pirogue), à ajuster. Prix du carburant : au Gabon le prix est fixé par l'État et change rarement — mettez ici le prix officiel en vigueur, il sera utilisé partout dans l'appli.</p>
