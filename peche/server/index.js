...
import { Trips, Tracks, Settings, getSettings, Users, Expenses, Pannes } from "./db.js";
...
app.get("/api/pannes", requireAuth, requireAdmin, (req, res) => {
  res.json(Pannes.all.all().map((p) => ({ ...p, photos: safeParse(p.photos, []) })));
});
...
app.post("/api/pannes", requireAuth, requireAdmin, async (req, res) => {
  ...
  try {
    for (const ph of (b.photos || [])) {
      if (ph) photoRefs.push(await savePhoto(ph));
    }
  ...
  const row = {
    ...
    photos: JSON.stringify(photoRefs), expense_id: null, created_at: Date.now(),
  };
  ...
  if (cout > 0) {
    const exp = { id: randomUUID(), date: b.date, type: "panne",
      label: "Réparation : " + row.titre, amount: cout, trip_id: null, created_at: Date.now() };
...
app.put("/api/pannes/:id", requireAuth, requireAdmin, (req, res) => {
  ...
  if (cout > 0) {
    const exp = { id: randomUUID(), date: cur.date, type: "panne",
      label: "Réparation : " + (b.titre ?? cur.titre), amount: cout, trip_id: null, created_at: Date.now() };
    Expenses.insert.run(exp);
...
app.get("/api/tresorerie", requireAuth, requireAdmin, (req, res) => {
  const trips = Trips.all.all().map((r) => deserTrip(r, false));
  const recettes = trips.reduce((s, t) =>
    s + (t.prises || []).reduce((a, p) => a + (p.kg || 0) * (p.prix || 0), 0), 0);
  const depenses = Expenses.all.all().reduce((s, e) => s + (e.amount || 0), 0);
