...
CREATE TABLE IF NOT EXISTS expenses (
  id         TEXT PRIMARY KEY,
  date       TEXT NOT NULL,
  type       TEXT NOT NULL,          -- carburant | paye | materiel | transport | licence | autre
  label      TEXT,
...
export const Trips = {
  ...
  byOwner: db.prepare("SELECT * FROM trips WHERE owner = ? ORDER BY date DESC, depart DESC"),
  one: db.prepare("SELECT * FROM trips WHERE id = ?"),
  insert: db.prepare(`INSERT INTO trips (id,date,depart,retour,zone,crew,prises,par,note,owner,locked,photo,gps_lat,gps_lon,created_at)
                      VALUES (@id,@date,@depart,@retour,@zone,@crew,@prises,@par,@note,@owner,@locked,@photo,@gps_lat,@gps_lon,@created_at)`),
  update: db.prepare(`UPDATE trips SET date=@date,depart=@depart,retour=@retour,zone=@zone,crew=@crew,prises=@prises,par=@par,note=@note WHERE id=@id`),
